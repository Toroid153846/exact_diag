...
const _dim = Ref(1)
const _site = Ref(1)
export init, Op, spin_op, id, shift, site_flip, spin_flip, sum_j, block_diag, block_diag_energy, true_f, mat_gene_nonsparse, shift_z2, energy_same_check, block_diag_energy_shiftz2, sum_j_fixed
#region 初期化
...
function block_diag_energy_shiftz2(H::Union{Op,Matrix{ComplexF64}}, indices::Vector{Vector{Int}}, uf::UnionFinder{Int}, k::Int, cases_ops::Tuple{Function,Op}...)
  ...
  for ntheigen1 in ntheigens_sorted
    for i in 1:ma
      ...
      if length(cases_ops) == 1 || (!case1(i, ma))
        eigens1, eigenvectors1 = eigen(H_mat1)
        if ma == 2 && length(eigens1) >= 3 && i == 1
          print(energy_gap(eigens1), ",")
        end
      else
        # ここでcases_opsの2番目以降の要素を使ってブロック対角化を行う
...
for L in 3:14
  ...
  sym = "u1"
  uf = UnionFinder(8)
  H1 = sum_j_fixed(j -> 0.5 * spin_op('z', j) * spin_op('z', j + 1) - 0.5 * hj * (spin_op('+', j) + spin_op('-', j))+sqrt(3.0)*spin_op('z',j))
  ...
  block_diag_energy_shiftz2(H1, (true_f, spin_flip()))
end
println("")
# @time energy, _ = eigen(mat_gene_nonsparse(H3))
#length(energy)
#println(block_diag_energy(H2, collect(1:8), uf, (shift_z2, shift()), (true_f, site_flip()))[1])
# print(length(energy))
# println(length(block_diag_energy_shiftz2(H2, (shift_z2, shift()), (true_f, site_flip()))[1]))
# @time block_diag_energy_shiftz2(H3, (true_f, S_z))[1]
# @time block_diag_energy_shiftz2(H3, (true_f, S_z), (shift_z2, shift()))[1]
#energy_same_check(energy, block_diag_energy_shiftz2(H3, (true_f, S_z), (shift_z2, shift()), (true_f, site_flip()))[1])
# energy_same_check(energy, block_diag_energy(H2, (true_f, S_z))[1])